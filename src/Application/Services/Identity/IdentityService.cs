...
namespace CleanArchitecture.Blazor.Application.Services.Identity;
...
public class IdentityService : IIdentityService
{
    ...
    public async Task<MethodResult<TokenResponse>> LoginAsync(TokenRequest request, CancellationToken cancellation = default)
    {
        var user = await _userManager.FindByNameAsync(request.UserName!);
        if (user == null)
        {
            return await MethodResult<TokenResponse>.FailureAsync(new string[] { _localizer["User Not Found."] });
        }
        if (!user.IsActive)
        {
            return await MethodResult<TokenResponse>.FailureAsync(new string[] { _localizer["User Not Active. Please contact the administrator."] });
        }
        if (!user.EmailConfirmed)
        {
            return await MethodResult<TokenResponse>.FailureAsync(new string[] { _localizer["E-Mail not confirmed."] });
        }
        var passwordValid = await _userManager.CheckPasswordAsync(user, request.Password!);
        if (!passwordValid)
        {
            return await MethodResult<TokenResponse>.FailureAsync(new string[] { _localizer["Invalid Credentials."] });
        }
        ...
        return await MethodResult<TokenResponse>.SuccessAsync(response);
    }

    public async Task<MethodResult<TokenResponse>> RefreshTokenAsync(RefreshTokenRequest request, CancellationToken cancellation = default)
    {
        if (request is null)
        {
            return await MethodResult<TokenResponse>.FailureAsync(new string[] { _localizer["Invalid Client Token."] });
        }
