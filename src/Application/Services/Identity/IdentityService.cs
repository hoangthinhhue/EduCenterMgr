...
namespace CleanArchitecture.Blazor.Domain.Services.Identity;
...
public class IdentityService : IIdentityService
{
    ...
    public async Task<MethodResult<TokenResponse>> LoginAsync(TokenRequest request, CancellationToken cancellation = default)
    {
        ...
        if (user == null)
        {
            return MethodResult<TokenResponse>.ErrorBussiness( _localizer["User Not Found."]);
        }
        if (!user.IsActive)
        {
            return MethodResult<TokenResponse>.ErrorBussiness( _localizer["User Not Active. Please contact the administrator."]);
        }
        if (!user.EmailConfirmed)
        {
            return MethodResult<TokenResponse>.ErrorBussiness( _localizer["E-Mail not confirmed."]);
        }
        var passwordValid = await _userManager.CheckPasswordAsync(user, request.Password!);
        if (!passwordValid)
        {
            return MethodResult<TokenResponse>.ErrorBussiness( _localizer["Invalid Credentials."]);
        }
    ...
    public async Task<MethodResult<TokenResponse>> RefreshTokenAsync(RefreshTokenRequest request, CancellationToken cancellation = default)
    {
        if (request is null)
        {
            return MethodResult<TokenResponse>.ErrorBussiness( _localizer["Invalid Client Token."]);
        }
